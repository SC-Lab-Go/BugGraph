...
def iterate_func():
    dict_func_id = {}
    index = 0
#    fp = open("func_assembly.csv", "w")
#    fp.write("func_id" + "," + "func_name" + ",func_addr" + "\n")
    for func in idautils.Functions():
        func_name = GetFunctionName(func)
        if func_name == "d2i_ECPrivateKey":
            with open(func_name + ".txt", "w") as fp:
                #fp.write(func_name + "\n")
                func_addr = hex(func)
#                fp.write(func_addr + ":" + func_name + '\n')
                for (startea, endea) in Chunks(func):
                    for head in Heads(startea, endea):
                        #fp.write("0x%08x"%(head) + ":\t" + GetDisasm(head) + '\n')
                        fp.write(GetDisasm(head) + '\n')
#                fp.write("\n")
#                fp.write(func_addr + ":" + GetDisasm(func_addr) + '\n')
    #        break
 #           get_func_assembly(func)
#        func_addr = hex(func)
