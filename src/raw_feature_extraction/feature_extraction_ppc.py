...
def check_transfer_instruc(m):
    ...
    arm_TI = {'b':1, 'bl':1, 'bx':1, 'blx':1, 'bxj':1, 'cb':1, 'tbb':1, 'tbh':1}
    ppc_TI = {'beq':1, 'bge':1, 'bgt':1, 'ble':1, 'blt':1, 'bne':1, 'bng':1, 'bnl':1, 'bns':1, 'bso':1, 'b':1, 'bc':1, 'bclr':1, 'bcctr':1, 'lt':1, 'gt':1, 'eq':1, 'so':1}

    transfer_set = {}
    transfer_set.update(x86_TI)
    transfer_set.update(mips_TI)
    transfer_set.update(arm_TI)
    transfer_set.update(ppc_TI)
...
def check_calls_instruc(m):
    call_set = {'call':1, 'jal':1, 'jalr':1, 'blr':1, 'bl':1}
    if m in call_set:
        return True
    return False
...
def check_logic_instruc(m):
    ...
    ppc_LI = {'and':1, 'andi':1, 'andis':1, 'cntlzw':1, 'eqv':1, 'extbs':1, 'extsh':1, 'nand':1, 'nor':1, 'or':1, 'ori':1, 'oris':1, 'slw':1, 'srw':1, 'srawi':1, 'sraw':1, 'xor':1, 'xori':1, 'xoris':1}
    arm_LI = {'lsl':1, 'lsr':1, 'tst':1, 'teq':1, 'and':1, 'eor':1, 'orr':1, 'orn':1, 'bic':1}

    logic_set = {}
    logic_set.update(x86_LI)
    logic_set.update(mips_LI)
    logic_set.update(arm_LI)
    logic_set.update(ppc_LI)
    if m in logic_set:
        return True
    return False
...
def check_arith_instruc(m):
    ...
    arm_AI = {'add':1, 'adc':1, 'asr':1, 'sub':1, 'sbc':1, 'rsb':1, 'rsc':1, 'mul':1, 'mla':1, 'mls':1, 'umull':1, 'umlal':1, 'umaal':1, 'smull':1, 'smlal':1, 'smulxy':1, 'smulwy':1, 'smlaxy':1, 'smlawy':1, 'smlalxy':1, 'smuad':1, 'smlad':1, 'smlald':1, 'smusd':1, 'smlsd':1, 'rm':1}
    ppc_AI = {'add':1, 'addi':1, 'addme':1, 'addze':1, 'neg':1, 'subf':1, 'subfic':1, 'subfme':1, 'subfze':1}

    arith_set = {}
    arith_set.update(x86_AI)
    arith_set.update(arm_AI)
    arith_set.update(mips_AI)
    arith_set.update(ppc_AI)
