...
def iterate_func():
    dict_func_id = {}
    index = 0
    fp = open("func_to_id_addr.csv", "w")
    fp.write("func_id" + "," + "func_name" + ",func_addr" + "\n")
    for func in idautils.Functions():
        func_name = GetFunctionName(func)
        func_addr = hex(func)
        dict_func_id[func_name] = index
        fp.write(str(index) + "," + func_name + "," + func_addr + "\n")
        index += 1
...
def get_instructions(func_name_to_id):

#    os.system("echo 'in fcg' >> debug.log")
    # traverse all the functions
    fp = open("instruction.csv", "w")
    for func in idautils.Functions():
        func_name = GetFunctionName(func)
        if func_name not in func_name_to_id:
            continue

        func_p = idaapi.get_func(func)
        addr = func_p.startEA;

        fp.write("function, {}, {}\n".format(addr, func_name))

#        addr = idc.NextHead(addr)
        while addr < func_p.endEA:
            fp.write("{}, {}\n".format(addr, GetDisasm(addr)))
            addr = idc.NextHead(addr)
...
#                block_key_addr[src_key] = (block.endEA, func_class.endEA)
            ...
            for succ_block in block.succs():
                ...
                if dest_key in block_key_id:
                    #Error: should be old id, not node_index
#                    cfg.add_edge(src_id, dest_id)
                    dest_id = block_key_id[dest_key]
                    cfg.add_edge(src_id, dest_id)
...
#    get_dfg(func_name_to_id, block_key_id, block_key_addr)
    ...
    get_instructions(func_name_to_id)
    idc.Exit(0)
